...
def place_draw_section(frame, xsEntry, ysEntry, draw_section, close_section, start_column):
    pointMakeLabel = tk.Label(frame, bg=MAIN_COLOUR_LABEL_BG, text="ПОСТРОЕНИЕ ФИГУРЫ",
                            font=("Consolas", FONT_HEAD),
                            fg=MAIN_COLOUR_LABEL_TEXT, relief=tk.SOLID)
    ...
    drawSectionBtn = tk.Button(frame, bg=MAIN_COLOUR, fg=MAIN_COLOUR_LABEL_TEXT, text="Построить вершину", font=("Consolas", FONT_BUTTON),
                            command=draw_section)
    drawCloseSectionBtn = tk.Button(frame, bg=MAIN_COLOUR, fg=MAIN_COLOUR_LABEL_TEXT, text="Замкнуть фигуру", font=("Consolas", FONT_BUTTON),
                            command=close_section)
    ...
    xsEntry.place(x=0, y=(start_column + 2) * FRAME_H // COLUMNS, width=FRAME_W // 2, height=FRAME_H // COLUMNS)
    ysEntry.place(x=FRAME_W // 2, y=(start_column + 2) * FRAME_H // COLUMNS, width=FRAME_W // 2, height=FRAME_H // COLUMNS)

    drawSectionBtn.place(x=0, y=(start_column + 3.5) * FRAME_H // COLUMNS, width=FRAME_W // 2, height=FRAME_H // COLUMNS)
    drawCloseSectionBtn.place(x=FRAME_W // 2, y=(start_column + 3.5) * FRAME_H // COLUMNS, width=FRAME_W // 2, height=FRAME_H // COLUMNS)
...
def place_mouse_mode(frame, clip, start_column):
    ...
    labelTextInfo_1 = tk.Label(frame, bg=FRAME_COLOUR, text="Левая кнопка - добавить вершину отсекателя",
                                font=("Consolas", FONT_LABEL),
                                fg=MAIN_COLOUR_LABEL_TEXT)
    labelTextInfo_2 = tk.Label(frame, bg=FRAME_COLOUR, text="Правая кнопка - добавить вершину фигуры",
                                font=("Consolas", FONT_LABEL),
                                fg=MAIN_COLOUR_LABEL_TEXT)

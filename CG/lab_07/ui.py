...
def place_draw_cliper(frame, xluEntry, yluEntry, xrdEntry, yrdEntry, draw_cliper, start_column):
    pointMakeLabel = tk.Label(frame, bg=MAIN_COLOUR_LABEL_BG, text="ПОСТРОЕНИЕ ОТСЕКАТЕЛЯ",
                            font=("Consolas", FONT_HEAD),
                            fg=MAIN_COLOUR_LABEL_TEXT, relief=tk.SOLID)

    msgAboutPoint = tk.Label(frame, bg=FRAME_COLOUR, text=("Xлв{0}Yлв{0}Xпн{0}Yпн".format(" " * int(FRAME_W // 6 // FONT_LABEL))),
                            font=("Consolas", FONT_LABEL),
                            fg=MAIN_COLOUR_LABEL_TEXT)

    drawSectionBtn = tk.Button(frame, bg=MAIN_COLOUR, fg=MAIN_COLOUR_LABEL_TEXT, text="Построить отсекатель", font=("Consolas", FONT_BUTTON),
                            command=draw_cliper)

    pointMakeLabel.place(x=0, y=start_column * FRAME_H // COLUMNS, width=FRAME_W, height=FRAME_H // COLUMNS)
    msgAboutPoint.place(x=0, y=(start_column + 1) * FRAME_H // COLUMNS, width=FRAME_W, height=FRAME_H // COLUMNS)

    xluEntry.place(x=0, y=(start_column + 2) * FRAME_H // COLUMNS, width=FRAME_W // 4, height=FRAME_H // COLUMNS)
    yluEntry.place(x=FRAME_W // 4, y=(start_column + 2) * FRAME_H // COLUMNS, width=FRAME_W // 4, height=FRAME_H // COLUMNS)
    xrdEntry.place(x=2 * FRAME_W // 4, y=(start_column + 2) * FRAME_H // COLUMNS, width=FRAME_W // 4, height=FRAME_H // COLUMNS)
    yrdEntry.place(x=3 * FRAME_W // 4, y=(start_column + 2) * FRAME_H // COLUMNS, width=FRAME_W // 4, height=FRAME_H // COLUMNS)

...
def getTypeOfMonotone(pointTable):
    isIncreasing = True
    isDecreasing = True
    for row in range(len(pointTable) - 1):
        if not (pointTable[row].x < pointTable[row + 1].x and pointTable[row].y >= pointTable[row + 1].y):
            isIncreasing = False
...
def calcApproxValue(tableOfSub, n, x):
    countOfArgs = 2
    sum = tableOfSub[1][0]
    mainPart = 1
    for i in range(n):
...
def rootByNewton(pointTable, n, monotone):
    ...
    if monotone != notMonotone:
        ...
        return calcApproxValue(subs, n, 0)
    else:
...
def rootByHermit(pointTable, n, monotone):
    if monotone != notMonotone:
        ...
        while r - 1 > 1e-8:
            m = (r + 1) / 2
            y = calcApproxValue(tableOfSub, n, m)
            if y < 0:
                l = m
            else:
                r = m
